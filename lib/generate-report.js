...
function generate() {
  ...
  const versionBenchFile = path.join(BENCHMARKS_DIR, 'benchmarks-all-3run.json');
  let versionResults = [];
  ...
  const startupFiles = fs.existsSync(BENCHMARKS_DIR) ?
    fs.readdirSync(BENCHMARKS_DIR).filter(f => f.match(/^benchmark-startup-.*\.json$/)) : [];
  const startupResults = [];

  startupFiles.forEach(file => {
    const data = JSON.parse(fs.readFileSync(path.join(BENCHMARKS_DIR, file), 'utf-8'));
  ...
  const outputFile = path.join(BENCHMARKS_DIR, 'STARTUP_COMPARISON.html');
  fs.mkdirSync(BENCHMARKS_DIR, { recursive: true });
  fs.writeFileSync(outputFile, html);
  console.log(`\n✅ Report generated: ${outputFile}`);
  console.log(`   Open in browser to view comparison charts\n`);

  return { html: outputFile, versionResults, interactiveResults: startupResults };
}

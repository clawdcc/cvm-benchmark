...
function getNextRunNumber() {
  if (!fs.existsSync(BENCHMARKS_DIR)) return '1';

  const runs = fs.readdirSync(BENCHMARKS_DIR)
...
function setupRunDirectory(runNum) {
  const runDir = path.join(BENCHMARKS_DIR, `run-${runNum}`);
...
async function runAll(options = {}) {
  ...
  try {
    ...
    console.log(`   Then copy ~/.cvm/benchmarks/benchmarks-all-3run.json to ${versionDir}/`);
